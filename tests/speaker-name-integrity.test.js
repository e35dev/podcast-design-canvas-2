...
test("applyHintsToText keeps confirmed setup names intact in surrounding copy", () => {
  ...
  const sample = "Sam Rivera welcomes Dana Kim to the show";

  assert.strictEqual(context.applyHintsToText(sample, review, "Host", "Sam Rivera"), sample);
  assert.strictEqual(context.applyHintsToText("Sam Rivera", review, "Host", "Sam Rivera"), "Sam Rivera");
});

test("createReview preserves setupName separately from spelling hints", () => {
  const episode = setup.summarize(draftWithSocial());
  const review = context.createReview(episode);
  assert.strictEqual(review.speakers[0].setupName, "Sam Rivera");
...
test("ACCEPTANCE: social context fixes transcript misspellings without rewriting setup names", () => {
  ...
  assert.strictEqual(contextReview.speakers[0].displayName, "Sam Rivera");
  ...
  let corrReview = correction.createCorrectionReview(episode, {
    contextReview: contextReview,
    momentsBoard: board,
  });
  corrReview = correction.approveCorrection(corrReview);
  ...
  const savedStore = templates.saveTemplate(
    templates.createStore(),
    templates.createTemplate("Founders template", applied.canvasDoc, "tpl-founders"),
  );
  assert.strictEqual(templates.getTemplate(savedStore, "tpl-founders").canvas.speakerFrames[0].name, "Sam Rivera");
});
