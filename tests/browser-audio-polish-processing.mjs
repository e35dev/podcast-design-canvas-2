...
(() => {
  ...
  async function run() {
    ...
    await wait();
    const applyButton = button(/Apply audio/);
    const continueButton = button(/^Continue/);
    log(Boolean(applyButton) && /Apply audio & continue/.test(applyButton.textContent || ""), "Audio flow stays on explicit Apply before completion");
    log(!continueButton, "Continue is not visible before Apply");
    log(!document.querySelector(".audio-track-status-complete"), "No saved tracks are shown before Apply");
