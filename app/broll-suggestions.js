...
(function (global) {
  ...
  function generateSuggestions(episodeSummary, correctionReview, momentsBoard) {
  ...
  function applyAccepted(board, session) {
    ...
    acceptedSuggestions(session).forEach((item) => {
      ...
      const created = VM.listMoments(nextBoard).slice(-1)[0];
      if (created) {
        item.momentId = created.id;
      }
    });
    return nextBoard;
  }
  ...
  function applyToBoard(board, session) {
    let nextBoard = applyAccepted(board, session);
    return {
      board: nextBoard,
