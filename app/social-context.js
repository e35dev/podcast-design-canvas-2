...
(function (global) {
  ...
  // Skip prefix hints (e.g. "Sam River" -> "Sam Rivera") when the text already
  // contains the confirmed canonical name. This avoids suffix corruption
  ...
  function shouldSkipHintReplacement(hint, canonical, text) {
    ...
    if (
      ...
      haystack.toLowerCase().indexOf(to.toLowerCase()) >= 0
    ) {
      return true;
    }
    return false;
