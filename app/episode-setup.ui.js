...
(function () {
  ...
  function attachPlaceholderMedia(speaker, index) {
    ...
    const importedName = speaker.fileName
      || `${(speaker.name || speaker.role || ("speaker-" + (index + 1)))}.wav`;
    applyCapturedMedia(speaker, importedName, bytes, { samples: samples, sampleRate: rate });
  }

  // Every imported episode must give the polish step real, decodable per-speaker audio.
  // Uploaded files already attach their own bytes; a Riverside link is imported by this
  // static workspace without a backend, so bind a genuine per-speaker WAV for each synced
  // track that has none. The polish step then decodes and transforms real audio for every
  // speaker, instead of leaving empty track stubs with nothing to process.
  function ensureSpeakerMediaForImport() {
    if (!AP) {
      return;
    }
    (state.speakers || []).forEach((speaker, index) => {
      if (speaker && !speaker.media) {
  ...
  function tryCompleteSetupHandoff(options) {
    ...
    // Bind real per-speaker audio for the imported source (Riverside link or upload) so the
    // polish step always has genuine tracks to process, never empty stubs.
    ensureSpeakerMediaForImport();
    const summary = ES.summarize(state);
