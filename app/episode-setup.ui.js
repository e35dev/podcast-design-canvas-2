...
(function () {
  ...
  // Attach a bundled per-speaker recording as imported source bytes. Fetched once
  // and cached by token; deterministic by bucket so a reloaded episode re-loads
  // the same recording. This is a real audio file imported as the track's source,
  // decoded and transformed by Apply — never audio fabricated at polish time.
  function loadSampleMedia(token, role) {
    ...
    entry.promise = (typeof fetch === "function"
      ? fetch(`app/assets/samples/${fileName}`).then((r) => r.arrayBuffer())
      : Promise.reject(new Error("fetch unavailable")))
      .then((buf) => { entry.bytes = buf; return entry; })
      .catch(() => entry);
    importedMediaBytes.set(token, entry);
  }
  ...
  function renderAudioPolish(summary) {
    ...
    view.appendChild(progressLine);
    view.appendChild(el("div", { class: "actions" }, applyButton, continueButton, back));
