...
(function () {
  ...
  function renderShowLibrary() {
    ...
    const galleryBtn = GAL && GAL.listListings(galleryStore).length
      ? el("button", { class: "btn-secondary", type: "button" }, "Creator template gallery →")
      : null;
    if (galleryBtn) {
      galleryBtn.addEventListener("click", () => renderCreatorGalleryBrowse(null, { returnTo: "library" }));
    }

    const actions = el("div", { class: "workspace-actions" }, newShowBtn, styleDemoBtn, galleryBtn, blankEpisodeBtn);
      ...
      view.appendChild(
        el(
          ...
          el(
            "p",
            { class: "hint" },
            "Save a show template from the canvas editor, then publish it here from the saved templates area.",
          ),
        ),
      );
      const back = el("button", { type: "button", class: "ghost" }, "← Back");
